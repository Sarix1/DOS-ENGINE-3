...
{
    switch (obj->controldata & (ACTION_LEFT|ACTION_RIGHT))
    {
        case ACTION_LEFT:
    ...
    switch (obj->controldata & (ACTION_UP|ACTION_DOWN))
    {
        case ACTION_UP:
    ...
    for (i = 1; i < g_Game.object_count; i++)
    {
            ...
            == DO_INTERSECT)
        {
            ...
            (
                ...
                g_Game.Objects[i].obb_w,
                g_Game.Objects[i].obb_h
            );
...
{
    int i = 0;
    PLAYER_OBJ->controldata = local_controldata; // ^ should be in a "logic" loop that  comes before physics, and includes AI, game logic, etc.
