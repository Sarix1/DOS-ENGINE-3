...
inline int16_t toInt(fixp i)
{
    return (i+FIXP_HALF) >> FIXP_SHIFT;
}
...
{
    return ((int64_t)a*b+FIXP_HALF) >> FIXP_SHIFT;
}

inline fixp fixpDiv(fixp num, fixp denom)
{
    return ((((int64_t)num) << FIXP_SHIFT)+(denom>>1)) / denom;
}

inline int64_t fixpSquare(fixp x)
{
    return ((int64_t)x*x+FIXP_HALF) >> FIXP_SHIFT;
}
